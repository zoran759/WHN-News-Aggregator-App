...
+function ($) { "use strict";
				...
				transition ?
					that.$element.find('.lightbox-dialog') // wait for modal to slide in
						.one($.support.transition.end, function ()
						{
						    ...
						    that.$element.find('.image-comments').css({
							'height': that.$element.find('img').height()-that.$element.find('.image-info').height()-61,
						    });
						})
	...
	Lightbox.prototype.preloadSize = function(callback)
	{
		...
		preloader.onload = function()
		{
		    ...
		    var comment_box_width=300;//TODO: this breaks mobile if image is bigger than window
			that.$element.find('.lightbox-dialog').css({
				'position': 'fixed',
				'width': preloader.width + padLeft + padRight + comment_box_width,
				'height': preloader.height + padTop + padBottom,
				'top' : (windowHeight / 2) - ( (preloader.height + padTop + padBottom) / 2),
				'left' : '50%',
				'margin-left' : -1 * (preloader.width + padLeft + padRight + comment_box_width) / 2
			});
			...
			that.$element.find('.image-comments').css({
				'width': comment_box_width-2,
			    'height': preloader.height-303,
			    'margin-left': 5
			});
			that.$element.find('.num-likes').css({
				'width': comment_box_width-2,
			    'margin-left': 5
			});
			$image.css({
